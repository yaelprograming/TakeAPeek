...
//         name: folderName,
//         parentFolderId: currentFolder || "0",
//         ownerId: userId,
//       })

//       console.log("Folder created successfully:", response.data)
//       fetchData(currentFolder)
//       enqueueSnackbar("התיקיה נוצרה בהצלחה!", { variant: "success" })
//     } catch (error) {
//       console.error("Error creating folder:", error)
//       enqueueSnackbar("שגיאה ביצירת התיקיה", { variant: "error" })
//     }
//   }
...
export function Gallery() {
  ...
  // Add state for create folder dialog
  ...
  const fetchData = async (folderId: string | null) => {
    ...
    try {
      ...
      console.log("Data fetched successfully- all data:", data)
      const userFolders = data.folders?.filter((folder: Folder) => !folder.isDeleted && folder.ownerId === userId) || []
      console.log("userFolders:", userFolders)
      const userFiles = data.files?.filter((file: ImageFile) => !file.isDeleted && file.ownerId === userId) || []
      console.log("userFiles:", userFiles)
      setFolders(userFolders)
      setFiles(userFiles)
      // Fetch breadcrumb
      const breadcrumbUrl = folderId ? `/folders/${folderId}/breadcrumb` : `/folders/0/breadcrumb`
